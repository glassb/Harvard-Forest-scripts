...
# ========= WEIGHTED-MEAN EXTRACT STATS FUNCTIONS FOR EMS TOWER

# returns spatial stats for a single FFP decDay 
EMS_extractStats_wm <- function(FFP_file,dayVar,rVar,rasterImage,path,fwm_file) {
  
  # EMS EASTING NORTHING VALUES
  
      # EASTING: xr+732275
      # NORTHING: yr+4713368
  
  # NEON EASTING NORTHING VALUES
...
EMS_extractStats_TCT_wm <- function(FFP_file,dayVar,rVar,rasterImage,path,fwm_file) {
...
# ========= WEIGHTED-MEAN EXTRACT STATS FUNCTIONS FOR NEON TOWER

# returns spatial stats for a single FFP decDay 
NEON_extractStats_wm <- function(FFP_file,dayVar,rVar,rasterImage,path,fwm_file) {
  ...
  fwm_file_filter <- fwm_file %>%
    filter(dec.day == dayVar) %>%
    mutate(f_value = as.numeric(f),
           easting = as.numeric(x+732180),
           northing=as.numeric(y+4713265))
...
# returns spatial stats for a single FFP decDay (for TCT files specifically)
NEON_extractStats_TCT_wm <- function(FFP_file,dayVar,rVar,rasterImage,path,fwm_file) {
