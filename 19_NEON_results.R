...
results <- merge(NEONmaster_mutated,results_mutated,by=c("Year.Year","time_days"))
...
ggplot(data = results_prime, aes(x=TCTb_mean,y=FC)) +
...
ggplot(data = results_prime, aes(x=TCTg_mean,y=FC)) +
  facet_wrap(~ month.Month) +
  geom_point(shape=20,alpha=.5,color="coral1") +
  scale_x_continuous() +
  scale_y_continuous() +
...
ggplot(data = results_prime, aes(x=TCTw_mean,y=FC)) +
  facet_wrap(~ month.Month) +
...
ggplot(data = results_prime) +
  ...
  geom_point(aes(x=time_days,y=TCTb_mean),shape=20,alpha=.8,color="coral1") +
  geom_point(aes(x=time_days,y=TCTg_mean),shape=20,alpha=.8,color="aquamarine3") +
  geom_point(aes(x=time_days,y=TCTw_mean),shape=20,alpha=.8,color="deepskyblue") +
...
ggplot(data = results_prime, aes(x=MGP_D_mean,y=FC)) +
  facet_wrap(~ month.Month) +
...
ggplot(data = results_prime, aes(x=MGP_C_mean,y=FC)) +
  facet_wrap(~ month.Month) +
